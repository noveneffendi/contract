...
rows = [["#{t 'nomor'}", "#{t 'sq_id'}", "#{t 'dates'}", "#{t 'customer'}", "#{t 'amount'}", "#{t 'disc'}", "#{t 'disc_rp'}", "#{t 'tax'}", "#{t 'tax_amount'}", "#{t 'total'}"]]
...
pdf.repeat(:all) do
	...
	pdf.number_pages "[#{t 'report'} of #{t 'sales_quotation'} yang Belum Selesai - #{t 'date_print'}: #{Date.today.strftime("%d/%m/%Y")} - #{t 'print_by'}: #{@employee}]", :size => 9, :at => [0, 0]
end
