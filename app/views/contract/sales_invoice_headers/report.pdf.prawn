...
grid([0,1], [0,4]).bounding_box do
	font("Times-Roman") do
		...
		pdf.text "#{t 'report'} #{t 'sales_invoice'}", :align => :center, :size => 13, :style => :bold
...
@sales_invoices.map do |row|
	if cnt == 0 || cnt.even?
		...
		rows +=
			[[
				row[0], {:content => "#{row[1]}", :colspan => 3 }, row[4], row[5], delimiter(row[6]), delimiter(row[7]), delimiter(row[8]), "#{row[9]}", delimiter(row[10]), delimiter(row[11])
			]]
	end
	cnt+=1
end
...
pdf.repeat(:all) do
	...
	pdf.number_pages "[#{t 'report'} #{t 'sales_invoice'} - #{t 'date_print'}: #{Date.today.strftime("%d/%m/%Y")} - #{t 'print_by'}: #{@employee}]", :size => 9, :at => [0, 0]
end
