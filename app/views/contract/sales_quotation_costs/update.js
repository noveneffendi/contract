$("#cst_table").html("<%= escape_javascript(render 'table') %>");

$("#sq_amount_<%= @sales_quotation_header.id %>").html("<%= @sales_quotation_header.currency.try(:code) %> <%= delimiter(@subtotal_amount.to_f) %>");
...
$("#sales_quotation_detail_modal").dialog("close");