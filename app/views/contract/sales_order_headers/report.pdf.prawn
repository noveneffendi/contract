...
grid([0,1], [0,4]).bounding_box do
	font("Times-Roman") do
		...
		pdf.text "#{t 'report'} #{t 'sales_order'}", :align => :center, :size => 13, :style => :bold
...
pdf.repeat(:all) do
	...
	pdf.number_pages "[#{t 'report'} of #{t 'sales_order'} - #{t 'date_print'}: #{Date.today.strftime("%d/%m/%Y")} - #{t 'print_by'}: #{@employee}]", :size => 9, :at => [0, 0]
end
